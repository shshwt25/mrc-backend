...
class PredictXLNetResponse():
	def __init__(self):
		...
		self.predict_xlnet_response_url = config['ServicesConfig']['predict_xlnet_response_url']
	...
	def on_post(self, req, resp):
		...
		data_xlnet_response_list = self.predict_xlnet_output(corpus_name, question, top_k_context, data_with_id)

		resp.status = falcon.HTTP_200
		resp.body = json.dumps(data_xlnet_response_list)
	# endDef
	...
	def predict_xlnet_output(self, corpus_name, question, top_k_context, data_with_id):
		try:
			corpusFiles = CorpusFiles()
			corpus_with_files_details = corpusFiles.get_corpus_files_list_by_corpus(corpus_name)

			data_xlnet_response_list = []
			if data_with_id['scores'][0] == 0:
				data_xlnet_response = {}
				data_xlnet_response['file_name'] = None
				data_xlnet_response['context'] = None
				data_xlnet_response['nlg'] = "Answer not available"
				data_xlnet_response['answer'] = "Answer not available"
				data_xlnet_response_list.append(data_xlnet_response)
			else:
				...
				for context_idx in range(top_k_context):
					...
					data_with_context = {'mode': "production", 'question': question,
										 ...
										 "model": "vanilla_xlnet"}
					data_xlnet_response = self.get_predict_response(json.dumps(data_with_context))
					print('Ending XLNet: ', time.ctime(time.time()))
					data_xlnet_response['file_name'] = data_context['file_name']
					data_xlnet_response['context'] = data_context['context_data']
					data_xlnet_response_list.append(data_xlnet_response)
			return data_xlnet_response_list
		except Exception as error:
			print("Error occurred in predict_xlnet_output", error)
	# endDef
	...
	def get_predict_response(self, data):
		try:
			#print('Calling xlnet service: ', data)
			res = requests.post(url=self.predict_xlnet_response_url,data=data, headers={'Content-Type': 'application/json', 'Connection':'close'})
